...
const DEVICES = [
  ...
  { type: 'splitter-8', label: '1:8 SPL', color: '#7c3aed', desc: '1:8 Passive Splitter' },
  { type: 'splitter-16', label: '1:16 SPL', color: '#7c3aed', desc: '1:16 Passive Splitter' },
  { type: 'splitter-32', label: '1:32 SPL', color: '#7c3aed', desc: '1:32 Passive Splitter' },
      ...
      <div style={{ display: 'flex', gap: 4, marginRight: 8, flexShrink: 0 }}>
        {DEVICES.map(d => (
          <div
            ...
            title={d.desc}
            style={{
              padding: '4px 8px', background: d.color + '22',
              border: `1px solid ${d.color}55`, borderRadius: 4,
              color: d.color, fontSize: 9, cursor: 'grab', userSelect: 'none',
